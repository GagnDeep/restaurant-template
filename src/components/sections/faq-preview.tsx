  ...
  return (
    <Section variant="muted" className="py-24 bg-muted/20">
      <div className="container px-4 md:px-6 max-w-3xl">
        <h2 className="text-3xl font-bold tracking-tighter sm:text-4xl md:text-5xl text-center mb-16 font-heading">
        ...
        <Accordion type="single" collapsible className="w-full space-y-4">
          {faq.questions.map((item, i) => (
            <AccordionItem key={i} value={`item-${i}`} className="bg-background border rounded-xl px-6 shadow-sm hover:shadow-md transition-all duration-200 data-[state=open]:border-primary/50">
              <AccordionTrigger className="text-lg font-bold text-left py-6 hover:no-underline font-heading [&[data-state=open]]:text-primary transition-colors">
                {item.question}
              </AccordionTrigger>
